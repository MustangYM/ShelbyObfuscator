...
string genHashedName(GlobalVariable *GV) {
    ...
    StringRef digestStr(reinterpret_cast<const char *>(digest.data()), digest.size());
    std::stringstream ss;
    ss << std::hex;
    for (size_t i = 0; i < digestStr.size(); i++) {
        ss << std::setw(2) << std::setfill('0') << (unsigned)(digestStr[i] & 0xFF);
    }
    return ss.str();
}
